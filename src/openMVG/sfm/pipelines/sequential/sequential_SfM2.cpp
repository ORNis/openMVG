...
bool SequentialSfMReconstructionEngine2::Process() {
  ...
  //--
  //- 2. While we can localize some cameras in the reconstruction
  //     a. call the Track Selection routine
  ...
  const std::array<float, 4> track_inlier_ratios = {0.2, 0.0};
  for (const float track_inlier_ratio : track_inlier_ratios) {
    while (AddingMissingView(track_inlier_ratio)) {
      // select best tracks untill convergence
      selectTracks(track_inlier_ratio);
...
bool SequentialSfMReconstructionEngine2::selectTracks(
        const float &track_inlier_ratio
) {
  Landmarks selected_tracks;

  double intersection_over_union = 0.0;
  while (true) //TODO: it's not clear if we do one last BA after "convergence" or not...
  {
    // Triangulate
    Triangulation();
    ...
    SfM_Data_Batched_Tracks_Selection ts(sfm_data_, shared_track_visibility_helper_);
    ts.setCoverage(100);
    ts.setTrackRatio(track_inlier_ratio);
    ...
    std::set_intersection(curr_selected_tracks_ids.cbegin(), curr_selected_tracks_ids.cend(),
                          prev_selected_tracks_ids.cbegin(), prev_selected_tracks_ids.cend(),
...
#endif
    {
      ...
      if (!track_id_for_resection.empty() && track_ratio > track_inlier_ratio) {
        ...
        const float inlier_ratio =
                resection_data.vec_inliers.size() / static_cast<float>(feature_id_for_resection.size());
        std::cout
                << std::endl
                << "-------------------------------" << "\n"
...
bool SequentialSfMReconstructionEngine2::BundleAdjustment(uint32_t max_num_iterations) {
